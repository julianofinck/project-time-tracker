def register_valid_registers(app, app_state):
    import datetime
    ...
    def update_hist_workhours(start_date, end_date, employee, project, product):
        ...
        # Project selected, Product is none
            ...
            if project == "CODEX":
                ...
                grouped = (
                    data.loc[mask, ["product", "hours"]].groupby(["product"]).sum()
                )
        ...
        # Project selected, Product selected
            ...
            if project == "CODEX":
                activity = product
                mask = (
                    mask & (data["project"] == project) & (data["activity"] == activity)
                )
                grouped = (
                    data.loc[mask, ["activity", "hours"]].groupby(["activity"]).sum()
                )
                ...
                mask = (
                    mask & (data["project"] == project) & (data["product"] == product)
                )
                grouped = (
                    data.loc[mask, ["activity", "hours"]].groupby(["activity"]).sum()
                )
